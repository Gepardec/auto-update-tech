...
# find all folders with build.gradle or build.gradle.kts, exclude build/, and save them sorted
modules=$(find "$PROJECT_ROOT" -type f \( -name "build.gradle" -o -name "build.gradle.kts" \) | grep -v "/build/" | while read -r buildfile; do
...
for module in $modules; do
  ...
  cd "$module" && mkdir -p "gepardec-reports" && LOG_FORMAT=json LOG_LEVEL=debug npx --prefix "$NODE_MODULES_PATH" renovate --platform=local --require-config=ignored --enabled-managers=gradle > "gepardec-reports/$RENOVATE_LOGFILE"
  grep "packageFiles with updates" "gepardec-reports/$RENOVATE_LOGFILE" > "gepardec-reports/$RENOVATE_FILTERED_LOGFILE"
